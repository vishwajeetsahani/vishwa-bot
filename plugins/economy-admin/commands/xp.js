    ...
    try {
      ...
      if (subcommand === 'add') {
        ...
        eventBus.publish('economyLog', {
          ...
          amount,
          context: { source: 'ADMIN', reason: `Added ${amount} XP` }
        });
        eventBus.publish('adminLog', {
          guildId,
          type: 'admin_xp_command',
          adminId: moderatorId,
          targetId: targetUser.id,
          context: { reward: `Added ${amount} XP` }
        });
      ...
      if (subcommand === 'remove') {
        ...
        eventBus.publish('economyLog', {
          ...
          userId: targetUser.id,
          amount,
          context: { source: 'ADMIN', reason: `Removed ${amount} XP` }
        });
        eventBus.publish('adminLog', {
          guildId,
          type: 'admin_xp_command',
          adminId: moderatorId,
          targetId: targetUser.id,
          context: { reward: `Removed ${amount} XP` }
        });
      ...
      if (subcommand === 'set') {
        ...
        eventBus.publish('economyLog', {
          ...
          userId: targetUser.id,
          amount,
          context: { source: 'ADMIN', reason: `Set XP to ${amount}` }
        });
        eventBus.publish('adminLog', {
          guildId,
          type: 'admin_xp_command',
          adminId: moderatorId,
          targetId: targetUser.id,
          context: { reward: `Set XP to ${amount}` }
        });
      ...
      if (subcommand === 'reset') {
        ...
        eventBus.publish('economyLog', {
          ...
          userId: targetUser.id,
          amount: 0,
          context: { source: 'ADMIN', reason: 'Reset XP and level' }
        });
        eventBus.publish('adminLog', {
          guildId,
          type: 'admin_xp_command',
          adminId: moderatorId,
          targetId: targetUser.id,
          context: { reward: 'Reset XP and level' }
        });
